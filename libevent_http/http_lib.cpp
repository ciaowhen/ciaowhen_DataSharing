...
int response_http(bufferevent *bev, const char *method, char *path)
{
    if(strcasecmp("GET", method) == 0)
    {
        ...
        {
            //应该显示目录列表
            send_header(bev, 200, "OK", get_file_type(".html"), sb.st_size);
            send_dir(bev, pf);
        }
        else        //处理文件
        {
...
int send_file_to_http(const char *filename, bufferevent *bev)
{
...
int send_dir(bufferevent *bev, const char *dirname)
{
    ...
    for(i = 0;i < num; i++)
    {
        ...
        printf("############ path = %s\n", path);
        if(lstat(path, &sb) < 0)
            sprintf(buf+ strlen(buf), "<tr><td><a href=\"%s\">%s</a></td></tr>\n", encoded_name, dirinfo[i]->d_name);
        else
        {
